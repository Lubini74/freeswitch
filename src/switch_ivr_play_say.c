													  ...
													  const char *tts_name, const char *voice_name, const char *text, switch_input_args_t *args)
{
	...
	if (need_create) {
		...
		if ((status = switch_core_speech_open(sh, tts_name, voice_name, (uint32_t) rate, interval, read_impl.number_of_channels, &flags, switch_core_session_get_pool(session))) != SWITCH_STATUS_SUCCESS) {
